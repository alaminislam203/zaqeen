...
// বাংলাদেশের পূর্ণাঙ্গ ৬৪ জেলার তালিকা
const districts = [
...
export default function CheckoutPage() {
    ...
    try {
      ...
      if (querySnapshot.empty) {
        ...
        if (!couponData.active) setCouponError("Coupon inactive");
          ...
          toast.success("Voucher credit applied successfully.");
        }
      }
  ...
  const handlePlaceOrder = async (e) => {
    ...
    if (!deliveryInfo.name || !deliveryInfo.address || deliveryInfo.phone.length < 11) {
        toast.error("Complete delivery identity required.");
        return;
    }
    if (paymentMethod !== 'cod' && (!transactionId || transactionId.length < 8)) {
        toast.error("Valid Transaction ID required.");
        return;
    }

    setOrderProcessing(true);
    const orderToast = toast.loading("Logging your acquisition to the blockchain...");
    ...
    try {
        ...
        // প্রিমিয়াম কনফার্মেশন মেসেজ
        toast.success("Order secured. An audit will follow shortly.", { 
            id: orderToast,
            duration: 5000,
        ...
        toast.error("Transmission failed. System breach detected.", { id: orderToast }); 
      ...
      <div className="max-w-7xl mx-auto px-6 py-12 md:py-24 animate-fadeIn">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 xl:gap-24">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-2">
                    ...
                    <select value={deliveryInfo.city} onChange={(e) => setDeliveryInfo({...deliveryInfo, city: e.target.value})} className="w-full bg-transparent border-b border-gray-100 py-3 text-[10px] font-black uppercase tracking-[0.2em] cursor-pointer outline-none focus:border-black">
                      <option value="Dhaka">Dhaka Metropolis</option>
            ...
            <section className="space-y-10 pt-10 border-t border-gray-50">
              ...
              <div className="bg-[#fcfcfc] border border-gray-50 rounded-sm p-10 space-y-10">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <button onClick={() => setPaymentMethod('bkash')} className={`py-5 rounded-sm text-[10px] font-black uppercase tracking-[0.3em] transition-all border ${paymentMethod === 'bkash' ? 'bg-[#D12053] border-[#D12053] text-white' : 'bg-white border-gray-100 text-gray-300'}`}>bKash Gateway</button>
                    <button onClick={() => setPaymentMethod('nagad')} className={`py-5 rounded-sm text-[10px] font-black uppercase tracking-[0.3em] transition-all border ${paymentMethod === 'nagad' ? 'bg-[#F7941E] border-[#F7941E] text-white' : 'bg-white border-gray-100 text-gray-300'}`}>Nagad Portal</button>
                    {deliveryInfo.city === 'Lakshmipur' && (
                        <div className="md:col-span-2">
                            <button onClick={() => setPaymentMethod('cod')} className={`w-full py-5 rounded-sm text-[10px] font-black uppercase tracking-[0.3em] transition-all border flex items-center justify-center gap-4 ${paymentMethod === 'cod' ? 'bg-emerald-500 border-emerald-500 text-white' : 'bg-white border-gray-100 text-gray-300'}`}>
                                <HiOutlineCash size={20}/> Cash on Delivery (Lakshmipur Only)
                            </button>
                        </div>
                    )}
                </div>
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-center gap-8">
                       <div className="space-y-3">
                          ...
                          <p className="text-[13px] font-bold leading-relaxed max-w-sm tracking-tighter">
                            Send <span className="text-2xl italic">৳{total.toFixed(0)}</span> to the secure wallet below.
                          </p>
                       </div>
                       <div className="bg-white px-8 py-5 rounded-sm border border-gray-100 flex items-center gap-8 shadow-sm">
                         <div>
                         ...
                         <button onClick={() => {navigator.clipboard.writeText("017XXXXXXXX"); toast.success("ID Copied")}} className="p-3 hover:bg-gray-50 rounded-full transition">
                            <HiOutlineClipboardCopy className="w-6 h-6 text-gray-300 hover:text-black" />
                         </button>
                       </div>
                    </div>
                    <div className="space-y-2">
          ...
          <div className="lg:col-span-5">
            <div className="sticky top-32 bg-white p-10 md:p-14 rounded-sm border border-gray-50 shadow-sm">
                ...
                <div className="pt-12 border-t border-gray-50 mt-12">
                   <div className="flex flex-col gap-8">
                      ...
                      <div className="flex bg-gray-50 rounded-sm overflow-hidden border border-gray-100 p-1.5">
                        ...
                        <button onClick={handleApplyCoupon} disabled={isApplying} className="bg-black text-white px-8 py-4 text-[9px] font-black uppercase tracking-[0.4em] hover:bg-gray-800 transition">{isApplying ? '...' : 'Validate'}</button>
                      </div>
              ...
              <div className="mt-16 space-y-8 border-t border-gray-50 pt-12">
                  <div className="flex items-center gap-5 group">
                      ...
                      <div>
                          <h4 className="text-[10px] font-black uppercase tracking-[0.3em]">Secure Transaction</h4>
                          <p className="text-[9px] text-gray-400 font-bold tracking-widest mt-1 italic leading-none">End-to-End Encrypted Audit</p>
                      </div>
                  </div>
                  <div className="flex items-center gap-5 group">
                      <HiOutlineTruck size={22} className="text-gray-300 group-hover:text-black transition-colors" />
                      <div>
                          <h4 className="text-[10px] font-black uppercase tracking-[0.3em]">Logistics Channel</h4>
                          <p className="text-[9px] text-gray-400 font-bold tracking-widest mt-1 italic leading-none">Dispatched in 48-72 Hours</p>
                      </div>
                  </div>
                  <div className="flex items-center gap-5 group">
                      <HiOutlineReply size={22} className="text-gray-300 group-hover:text-black transition-colors" />
                      <div>
                          <h4 className="text-[10px] font-black uppercase tracking-[0.3em]">Replacement Policy</h4>
                          <p className="text-[9px] text-gray-400 font-bold tracking-widest mt-1 italic leading-none underline">7-Day Verification Guaranteed</p>
                      </div>
                  </div>
              </div>
            </div>
